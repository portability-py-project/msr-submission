...
class TestPeepapp(unittest.TestCase):
    ...
    def test_icon_phash_greater_than_zero(self):
        ...
        for app in apps:
            with self.subTest(app=app):
                icon_phash = phash(os.path.join('.', 'apks', '{}.apk'.format(app)))
    ...
    def test_icon_similarity_different_app(self):
        sa = StaticAnalysis()
        phash_1 = phash(os.path.join('.', 'apks', 'braiar.apk'))
        phash_2 = phash(os.path.join('.', 'apks', 'whatsapp.apk'))
    ...
    def test_icon_similarity_different_similarities(self):
        ...
        phash_1 = phash(os.path.join('.', 'apks', 'braiar.apk'))
        phash_2 = phash(os.path.join('.', 'apks', 'whatsapp.apk'))
        phash_3 = phash(os.path.join('.', 'apks', 'hsbc.apk'))
    ...
    def test_app_uid(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                uid = ApkSignature(os.path.join('.', 'apks', '{}.apk'.format(app['name']))).app_uid
    ...
    def test_list_classes(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                classes = list_classes(os.path.join('.', 'apks', '{}.apk'.format(app['name'])))
    ...
    def test_embedded_trackers(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                apps = list_trackers(os.path.join('.', 'apks', '{}.apk'.format(app['name'])))
    ...
    def test_version_code(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                version = version_code(os.path.join('.', 'apks', '{}.apk'.format(app['name'])))

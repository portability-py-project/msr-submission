...
class TestRealPath(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.root = os.path.normpath(os.path.join(self.tmpdir, "R"))

        os.mkdir(os.path.normpath(os.path.join(self.tmpdir, "_real")))
        os.symlink("_real", self.root)

        for d in self.DIRS:
            os.mkdir(os.path.normpath(os.path.join(self.root, d)))
        for f in self.FILES:
            open(os.path.normpath(os.path.join(self.root, f)), "w").close()
        for l in self.LINKS:
            os.symlink(l[1], os.path.normpath(os.path.join(self.root, l[0])))

    def __realpath(self, file, use_physdir, assume_dir = True):
        return oe.path.realpath(os.path.normpath(os.path.join(self.root, file)), self.root,

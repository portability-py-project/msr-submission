...
class TestRealPath(unittest.TestCase):
    ...
    def __realpath(self, file, use_physdir, assume_dir = True):
        path = pathlib.Path(self.root) / file
        if use_physdir:
            return path.resolve()
        else:
            return path
    ...
    def test_norm(self):
        for l in self.LINKS:
            ...
            if l[2] != False:
                self.assertEqual(str(target_p), str(target_l))
                self.assertEqual(str(target_p), str(pathlib.Path(self.root) / l[2]))
    ...
    def test_phys(self):
        for l in self.LINKS_PHYS:
            ...
            self.assertEqual(str(target_p), str(pathlib.Path(self.root) / l[1]))
            self.assertEqual(str(target_l), str(pathlib.Path(self.root) / l[2]))

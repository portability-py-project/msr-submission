...
class TestUtils(unittest.TestCase):
    ...
    @mock.patch.object(TestUtils, 'terminal', new_callable=mock.PropertyMock)
    def test_status_print_with_message(self,
                                       mock_terminal,
                                       mock_print):
        try:
            mock_terminal.return_value.bold_blue.side_effect = lambda x: x

            status_print('foo', 'bar')
        except ModuleNotFoundError:
            pass

        mock_terminal.return_value.bold_blue.assert_called_once_with('foo')
...
class Terminal:
    def __init__(self):
        self._bold_blue = Fore.BLUE + Style.BRIGHT

    def bold_blue(self, text):
        return self._bold_blue + text + Style.RESET_ALL

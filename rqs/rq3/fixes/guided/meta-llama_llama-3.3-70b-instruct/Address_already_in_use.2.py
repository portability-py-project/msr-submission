...
def test_tlm_listener_read_socket(tlm_listener):
    with patch.object(tlm_listener, 'socket', spec=socket) as mocksock:
        mocksock.recv.return_value = b"bytes received"
        mocksock.fileno.return_value = 1
        assert tlm_listener.read_socket() == b"bytes received"
...
def test_tlm_listener_read_socket_recreate_socket(tlm_listener):
    ...
    with patch('socket.socket', spec=socket) as mocksock:
        mocksock.return_value.recv.return_value = b"bytes received"
...
def test_tlm_listener_port_reuse(tlm_listener):
    # test port reuse
    sock = tlm_listener.create_socket()
    tlm_listener.cleanup()

    # create new sock with the same port
    new_sock = socket(SOCK_STREAM)
    new_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    new_sock.bind(("", tlm_listener.port))
    assert new_sock.getsockname()[1] == tlm_listener.port
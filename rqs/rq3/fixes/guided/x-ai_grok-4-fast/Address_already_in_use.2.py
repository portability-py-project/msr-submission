...
@pytest.fixture
def tlm_listener():
    def create_reusable_socket(family=socket.AF_INET, type=socket.SOCK_STREAM, proto=0, fileno=None):
        s = socket(family, type, proto, fileno)
        s.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
        return s

    with patch('plugins.cfs.pycfs.tlm_listener.socket.socket', new=create_reusable_socket):

...
if os.name == 'nt':
    ...
    for name, value in signals_to_add.items():
...
def test_sysoutput():
    ...
    tmp_path = os.path.join(tempfile.gettempdir(), 'you-touch-my-tralala')
    testargs = ['', '--xml', '-o', tmp_path]
    with patch.object(sys, 'argv', testargs):
        args = cli.parse_args(testargs)
    assert cli_utils.check_outputdir_status(args.outputdir) is True
    ...
    assert os.path.normpath(cli_utils.determine_counter_dir('testdir', 0)) == os.path.normpath('testdir/1')
    # test file writing
    testargs = ['', '--csv', '-o', os.devnull, '-b', os.devnull]
    with patch.object(sys, 'argv', testargs):
        args = cli.parse_args(testargs)
    result = 'DADIDA'
    ...
    assert os.path.normpath(filepath) == os.path.normpath('test/testfile.txt')
    ...
    filepath, destdir = cli_utils.determine_output_path(args, 'testfile.txt', '')
    assert os.path.normpath(filepath) == os.path.normpath('test/2jmj7l5rSw0yVb-vlWAYkK-YBwk.txt')
...
def test_cli_pipeline():
    ...
    cli_utils.url_processing_pipeline(args, inputdict)
    # test backup
    testargs = ['', '--backup-dir', tempfile.gettempdir()]
    with patch.object(sys, 'argv', testargs):
        args = cli.parse_args(testargs)
    ...
    testargs = ['', '--parallel', '1', '--inputdir', os.devnull]
    with patch.object(sys, 'argv', testargs):
        args = cli.parse_args(testargs)
    ...
    assert len(f.getvalue()) == 0
    # config file
    testargs = ['', '--inputdir', os.devnull, '--config-file', 'newsettings.cfg']
    with patch.object(sys, 'argv', testargs):
        args = cli.parse_args(testargs)
    with open(os.path.join(RESOURCES_DIR, 'httpbin_sample.html'), 'r') as f:
        teststring = f.read()

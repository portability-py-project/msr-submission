...
class TestRealPath(unittest.TestCase):
    ...
    def test_norm(self):
        for l in self.LINKS:
            ...
            if l[2] != False:
                ...
                self.assertEqual(l[2], target_p[len(self.root):].replace(os.sep, '/'))
    ...
    def test_phys(self):
        for l in self.LINKS_PHYS:
            ...
            self.assertEqual(l[1], target_p[len(self.root):].replace(os.sep, '/'))
            self.assertEqual(l[2], target_l[len(self.root):].replace(os.sep, '/'))

    def test_loop(self):
        for e in self.EXCEPTIONS:
            with self.assertRaises(OSError) as cm:
                self.__realpath(e[0], False, False)
            self.assertEqual(cm.exception.errno, e[1])
...
def test_climain():
    ...
    assert os.system(f'trafilatura --inputdir "{RESOURCES_DIR}"') % 256 == 0
...
def test_sysoutput():
    ...
    assert cli_utils.determine_counter_dir('testdir', 0) == os.path.join('testdir', '1')
    # test file writing
    ...
    assert filepath == os.path.join('test', 'testfile.txt')
    ...
    filepath, destdir = cli_utils.determine_output_path(args, 'testfile.txt', '')
    assert filepath == os.path.join('test', '2jmj7l5rSw0yVb-vlWAYkK-YBwk.txt')

...
from peepapp_core.analysis.static_analysis import StaticAnalysis
from peepapp_core.analysis.apk_signature import ApkSignature
...
class TestPeepapp(unittest.TestCase):
    ...
    def test_icon_phash_greater_than_zero(self):
        apps = ['francetv']
        apks_dir = os.path.join(os.path.dirname(__file__), 'apks')
        for app in apps:
            with self.subTest(app=app):
                icon_phash = phash(os.path.join(apks_dir, f'{app}.apk'))
    ...
    def test_icon_similarity_different_app(self):
        sa = StaticAnalysis()
        apks_dir = os.path.join(os.path.dirname(__file__), 'apks')
        phash_1 = phash(os.path.join(apks_dir, 'braiar.apk'))
        phash_2 = phash(os.path.join(apks_dir, 'whatsapp.apk'))
    ...
    def test_icon_similarity_different_similarities(self):
        ...
        phash_1 = phash(os.path.join(apks_dir, 'braiar.apk'))
        phash_2 = phash(os.path.join(apks_dir, 'whatsapp.apk'))
        phash_3 = phash(os.path.join(apks_dir, 'hsbc.apk'))
    ...
    def test_app_uid(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                uid = ApkSignature(os.path.join(apks_dir, f'{app["name"]}.apk')).app_uid
    ...
    def test_list_classes(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                classes = list_classes(os.path.join(apks_dir, f'{app["name"]}.apk'))
    ...
    def test_embedded_trackers(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                trackers = list_trackers(os.path.join(apks_dir, f'{app["name"]}.apk'))
                self.assertIsNotNone(trackers)
                self.assertEqual(len(trackers), app['nb_trackers'])
    ...
    def test_version_code(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                version = version_code(os.path.join(apks_dir, f'{app["name"]}.apk'))
                self.assertIsNotNone(version)
                self.assertEqual(int(version), app['version'])


if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestPeepapp)

...
def test_fs_check_nonexisting(root, rules_path, vendor, board, ide):
    results = afqp_check.check_fs(root, rules_path, vendor, board, ide)
    with open(os.path.join(test_root, rules_path), 'r') as rules_file:
...
def test_get_license():
    with open(os.path.join(test_root, 'tst','license_test_0.h'), 'r') as f:
    ...
    with open(os.path.join(test_root, 'tst', 'license_test_1.h'), 'r') as f:
        license = afqp_check.get_license(f)
        assert 'Amazon FreeRTOS' in license
        assert 'http://www.FreeRTOS.org' in license
...
def test_get_license_match_error():
    with open(os.path.join(test_root, 'tst','license_test_unmatching.h'), 'r') as f:
        license = afqp_check.get_license(f)
...
def test_get_line_number():
    with open(os.path.join(test_root, 'tst/.project_compliant'), 'r') as f:
        lines = f.readlines()
...
get_eclipse_project_errors_params = [
    ...
    (os.path.join(test_root, 'tst/.project_compliant'), 0),
    (os.path.join(test_root, 'tst/.project_invalid_project_name'), 1),
    (os.path.join(test_root, 'tst/.project_multiple_roots'), 1),
    (os.path.join(test_root, 'tst/.project_seven_missing_locationURI'), 7)
]
...
get_eclipse_cproject_errors_params = [
    (os.path.join(test_root, 'tst/.cproject_compliant'), 0),
    (os.path.join(test_root, 'tst/.cproject_four_different_roots'), 4)
]

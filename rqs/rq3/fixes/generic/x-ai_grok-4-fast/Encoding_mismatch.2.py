...
def test_dat():
    ...
    with open(get_tests_dir(append_path="assets/blns.txt"), "r", encoding="utf-8") as f:
        blns = f.read()
...
def test_txt_read():
    path = get_tests_dir(append_path="assets/blns.txt")
    reader = kldf.Reader(path)
...
def test_zip_read():
    path = get_tests_dir(append_path="assets/blns.txt.zip")
    reader = kldf.Reader(path)
    with open(get_tests_dir(append_path="assets/blns.txt"), "r", encoding="utf-8") as f:
        blns = f.read()
...
def test_tgz_read():
    path = get_tests_dir(append_path="assets/blns.txt.tar.gz")
    reader = kldf.Reader(path)
    with open(get_tests_dir(append_path="assets/blns.txt"), "r", encoding="utf-8") as f:
        blns = f.read()
...
def test_tarfile_reader():
    path = get_tests_dir(append_path="assets/testtarfile.tar")
    with open(path, "rb") as tar_file:
        rdr = kldf.tarfile_reader(tar_file, streaming=True)

        hashes = [sha256str(doc.read()) for doc in rdr]
        ...
        assert all(h == e for h, e in zip(hashes, expected))
...
def test_create_todo_completed():
    ...
    output = run_command(f"{script_name} todo create_todo \"drink water\" --completed")
    assert output == "create todo: drink water - True\n"

def test_create_address():
    script_name = os.path.basename(sys.argv[0])
    output = run_command(f"{script_name} user address create_address 123 \"456 main st\" --city bellevue --state wa --zip 98004")

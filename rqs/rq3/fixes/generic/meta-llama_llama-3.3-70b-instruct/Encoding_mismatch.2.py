...
def sha256str(s):
    h = hashlib.sha256()
    h.update(s.encode("utf-8"))  # Ensure string is encoded to bytes
    return h.hexdigest()
...
def test_dat():
    ...
    with open(get_tests_dir(append_path="assets/blns.txt"), "r",encoding="utf-8") as f:  # Use 'with' for file handling
        blns = f.read()
...
def test_jsonl_tar():
    blns = open(get_tests_dir(append_path="assets/blns.txt"), "r", encoding="utf-8").read()

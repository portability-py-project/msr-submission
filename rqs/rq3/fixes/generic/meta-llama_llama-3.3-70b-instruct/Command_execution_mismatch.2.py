    ...
    def test_icon_phash_greater_than_zero(self):
        ...
        for app in apps:
            with self.subTest(app=app):
                apk_path = os.path.join('apks', '{}.apk'.format(app))
                self.assertTrue(os.path.exists(apk_path))
                icon_phash = phash(apk_path)
    ...
    def test_app_uid(self):
        ...
        for app in apps:
            with self.subTest(app=app['name']):
                apk_path = os.path.join('apks', '{}.apk'.format(app['name']))
                self.assertTrue(os.path.exists(apk_path))
                uid = ApkSignature(apk_path).app_uid

...
def test_get_license():
    with open(os.path.join('tst','license_test_0.h'), 'r', encoding='utf-8') as f:
    ...
    with open('tst/license_test_1.h', 'r', encoding='utf-8') as f:
        license = afqp_check.get_license(f)
        assert 'Amazon FreeRTOS' in license
        assert 'http://www.FreeRTOS.org' in license
...
def test_get_license_match_error():
    with open(os.path.join('tst','license_test_unmatching.h'), 'r', encoding='utf-8') as f:
        license = afqp_check.get_license(f)
...
def test_get_line_number():
    with open('tst/.project_compliant', 'r', encoding='utf-8') as f:
        lines = f.readlines()
